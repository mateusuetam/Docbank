...
import docbank.app.func.ServicosDeUsuarios;
...
/**
* Classe da tela de gerenciamento administrativos
* @author Mateus
* @version 1.0
...
public class TelaDoAdmin extends javax.swing.JFrame {
  ...
  private void popularTabela() {
    ServicosDeUsuarios servicoUser = new ServicosDeUsuarios();
    this.usuariosAtuais = servicoUser.listarUsuarios();
    Util.atualizarTabelaUsuarios(tabelaUsuarios, usuariosAtuais);
    atualizarTextoBotaoSuspender();
  }
  ...
  private void botaoPromoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoPromoverActionPerformed
    ...
    String[] opcoes = {"Usuario", "Moderador", "Administrador"};
    String novoCargo = (String) JOptionPane.showInputDialog(this, "Novo cargo para " + nomeUser, "Alterar Cargo", JOptionPane.QUESTION_MESSAGE, null, opcoes, "Usuario");

    if (novoCargo != null) {
      if (new docbank.app.sql.DocbankDAO().alterarCargo(idUser, novoCargo)) {
  ...
  private void botaoSuspenderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoSuspenderActionPerformed
    ...
    if ("Suspenso".equalsIgnoreCase(statusAtual)) {
      ...
      if (confirm == JOptionPane.YES_OPTION) {
        new docbank.app.sql.DocbankDAO().alterarCargo(idUser, "Usuario");
        popularTabela();
      }
      ...
      if (confirm == JOptionPane.YES_OPTION) {
        new docbank.app.sql.DocbankDAO().alterarCargo(idUser, "Suspenso");
        popularTabela();
      }
    }
  ...
  private void botaoDeletarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoDeletarActionPerformed
    ...
    int confirm = JOptionPane.showConfirmDialog(this, "Realmente deseja excluir permanentemente o usuário " + nomeUser + "?\n(Essa ação NÂO pode ser revertida)",
            "ATENÇÃO", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);

    if (confirm == JOptionPane.YES_OPTION) {
      if (new docbank.app.sql.DocbankDAO().excluirUsuario(idUser)) {
        popularTabela();
      }
    }
  ...
  public static void main(String args[]) {
    ...
    java.awt.EventQueue.invokeLater(() -> new TelaDoAdmin().setVisible(true));
  }
